...
public class GoogleHashSet<E> extends AbstractSet<E> implements Serializable {
  ...
  public GoogleHashSet(GoogleHashSet<E> s) {
    table = new Object[s.table.length];
    System.arraycopy(s.table, 0, table, 0, s.table.length);
    size = s.size;
  }
  ...
  public GoogleHashSet(int expectedSize) {
      int newCapacity = INITIAL_TABLE_SIZE;
      while (newCapacity * 3 < expectedSize * 4) {
        newCapacity <<= 1;
      }
  ...
  public void ensureSizeForAdditional(int additionalSize) {
      ensureSizeFor(size + additionalSize);
  }
